...
import {
  getStarkKey,
  ethSigToPrivate,
  sign as starknetCurveSign,
} from "../static_dependencies/scure-starknet/index.js";
...
export default class Exchange {
  ...
  constructor(userConfig = {}) {
    ...
    this.newUpdates =
      (this.options as any).newUpdates !== undefined
        ? (this.options as any).newUpdates
        : true;
  ...
  setProxyAgents(httpProxy, httpsProxy, socksProxy) {
    ...
    if (!isNode && (httpProxy || httpsProxy || socksProxy)) {
      throw new NotSupported(
        this.id +
          ' - proxies in browser-side projects are not supported. You have several choices: [A] Use `exchange.proxyUrl` property to redirect requests through local/remote cors-proxy server (find sample file named "sample-local-proxy-server-with-cors" in https://github.com/ccxt/ccxt/tree/master/examples/ folder, which can be used for REST requests only) [B] override `exchange.fetch` && `exchange.watch` methods to send requests through your custom proxy'
      );
    }
    if (httpProxy) {
      if (this.httpProxyAgentModule === undefined) {
        throw new NotSupported(
          this.id +
            " you need to load JS proxy modules with `.loadProxyModules()` method at first to use proxies"
        );
      }
      if (!(httpProxy in this.proxyDictionaries)) {
        this.proxyDictionaries[httpProxy] =
          new this.httpProxyAgentModule.HttpProxyAgent(httpProxy);
      }
      ...
      if (!(httpsProxy in this.proxyDictionaries)) {
        this.proxyDictionaries[httpsProxy] =
          new this.httpsProxyAgentModule.HttpsProxyAgent(httpsProxy);
      }
      ...
      if (this.socksProxyAgentModule === undefined) {
        throw new NotSupported(
          this.id +
            ' - to use SOCKS proxy with ccxt, at first you need install module "npm i socks-proxy-agent" and then initialize proxies with `.loadProxyModules()` method'
        );
      }
      if (!(socksProxy in this.proxyDictionaries)) {
        this.proxyDictionaries[socksProxy] =
          new this.socksProxyAgentModule.SocksProxyAgent(socksProxy);
      }
    ...
    if (isNode) {
      ...
      if (url.substring(0, 5) === "ws://") {
        if (this.httpAgent === undefined) {
          throw new NotSupported(
            this.id +
              " to use proxy with non-ssl ws:// urls, at first run  `await exchange.loadHttpProxyAgent()` method"
          );
        }
        return this.httpAgent;
      }
  ...
  async fetch(
    ...
    // user-agent
    const userAgent =
      this.userAgent !== undefined ? this.userAgent : this.user_agent;
    if (userAgent && isNode) {
      ...
      if (isNode) {
        ...
        try {
          ...
          if (typeof fetch === "function") {
            ...
            throw new Error(
              'Seems, "fetch" function is not available in your node-js version, please use latest node-js version'
            );
          }
        }
      } else {
    ...
    try {
      ...
      if (e instanceof this.AbortError) {
        ...
        throw new NetworkError(
          this.id + " " + method + " " + url + " fetch failed"
        );
      }
      throw e;
    }
  ...
  client(url: string): WsClient {
    ...
    if (!this.clients[url]) {
      ...
      const chosenAgent = this.setProxyAgents(
        httpProxy,
        httpsProxy,
        socksProxy
      );
      ...
      const options = this.deepExtend(
        this.streaming,
        {
          ...
          ping: (this as any).ping
            ? (this as any).ping.bind(this)
            : (this as any).ping,
  ...
  watchMultiple(
    ...
    const future = Future.race(
      messageHashes.map((messageHash) => client.future(messageHash))
    );
  ...
  async loadOrderBook(
    ...
    if (!(symbol in this.orderbooks)) {
      client.reject(
        new ExchangeError(
          this.id + " loadOrderBook() orderbook is not initiated"
        ),
        messageHash
      );
      return;
    }
    const maxRetries = this.handleOption(
      "watchOrderBook",
      "snapshotMaxRetries",
      3
    );
    ...
    try {
      ...
      while (tries < maxRetries) {
        ...
        const orderBook = await this.fetchRestOrderBookSafe(
          symbol,
          limit,
          params
        );
      ...
      client.reject(
        new ExchangeError(
          this.id +
            " nonce is behind the cache after " +
            maxRetries.toString() +
            " tries."
        ),
        messageHash
  ...
  ethEncodeStructuredData(domain, messageTypes, messageData) {
    return this.base16ToBinary(
      TypedDataEncoder.encode(domain, messageTypes, messageData).slice(-132)
    );
  }
  ...
  starknetEncodeStructuredData(domain, messageTypes, messageData, address) {
    ...
    if (types.length > 1) {
      throw new NotSupported(
        this.id + "starknetEncodeStructuredData only support single type"
      );
    }
    const request = {
      domain: domain,
  ...
  safeDictN(
    dictionaryOrList,
    keys: IndexType[],
    defaultValue: Dictionary<any> = undefined
  ...
  checkProxyUrlSettings(
    ...
    if (length > 1) {
      ...
      throw new InvalidProxySettings(
        this.id +
  ...
  checkProxySettings(
    ...
    const ishttpProxyCallbackDefined = this.valueIsDefined(
      this.httpProxyCallback
    );
    const ishttp_proxy_callback_defined = this.valueIsDefined(
      this.http_proxy_callback
    );
    if (ishttpProxyCallbackDefined || ishttp_proxy_callback_defined) {
      usedProxies.push("httpProxyCallback");
      httpProxy = ishttpProxyCallbackDefined
        ? this.httpProxyCallback(url, method, headers, body)
        : this.http_proxy_callback(url, method, headers, body);
    }
    // httpsProxy
    ...
    const ishttpsProxyCallbackDefined = this.valueIsDefined(
      this.httpsProxyCallback
    );
    const ishttps_proxy_callback_defined = this.valueIsDefined(
      this.https_proxy_callback
    );
    if (ishttpsProxyCallbackDefined || ishttps_proxy_callback_defined) {
      usedProxies.push("httpsProxyCallback");
      httpsProxy = ishttpsProxyCallbackDefined
        ? this.httpsProxyCallback(url, method, headers, body)
        : this.https_proxy_callback(url, method, headers, body);
    }
    // socksProxy
    ...
    const issocksProxyCallbackDefined = this.valueIsDefined(
      this.socksProxyCallback
    );
    const issocks_proxy_callback_defined = this.valueIsDefined(
      this.socks_proxy_callback
    );
    if (issocksProxyCallbackDefined || issocks_proxy_callback_defined) {
      usedProxies.push("socksProxyCallback");
      socksProxy = issocksProxyCallbackDefined
        ? this.socksProxyCallback(url, method, headers, body)
        : this.socks_proxy_callback(url, method, headers, body);
    }
    // check
    const length = usedProxies.length;
    if (length > 1) {
    ...
    if (this.valueIsDefined(limit)) {
      ...
      if (arrayLength > 0) {
        ...
        if (fromStart) {
          ...
          array = ascending
            ? this.arraySlice(array, 0, limit)
            : this.arraySlice(array, -limit);
        } else {
          array = ascending
            ? this.arraySlice(array, -limit)
            : this.arraySlice(array, 0, limit);
  ...
  filterByValueSinceLimit(
    ...
    if (valueIsDefined || sinceIsDefined) {
      ...
      for (let i = 0; i < parsedArray.length; i++) {
        ...
        const entryKeyGESince =
          entryKeyValue && since !== undefined && entryKeyValue >= since;
  ...
  async watchLiquidations(
    ...
    if (this.has["watchLiquidationsForSymbols"]) {
      return await this.watchLiquidationsForSymbols(
        [symbol],
        since,
        limit,
        params
    ...
    throw new NotSupported(
      this.id + " watchLiquidations() is not supported yet"
    );
  }

  async watchLiquidationsForSymbols(
    symbols: string[],
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " watchLiquidationsForSymbols() is not supported yet"
    );
  }

  async watchMyLiquidations(
    symbol: string,
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " watchMyLiquidations() is not supported yet"
    );
  }

  async watchMyLiquidationsForSymbols(
    symbols: string[],
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " watchMyLiquidationsForSymbols() is not supported yet"
    );
  }
  ...
  async watchTradesForSymbols(
    ...
    throw new NotSupported(
      this.id + " watchTradesForSymbols() is not supported yet"
    );
  }

  async unWatchTradesForSymbols(symbols: string[], params = {}): Promise<any> {
    throw new NotSupported(
      this.id + " unWatchTradesForSymbols() is not supported yet"
    );
  }

  async watchMyTradesForSymbols(
    symbols: string[],
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " watchMyTradesForSymbols() is not supported yet"
    );
  }

  async watchOrdersForSymbols(
    symbols: string[],
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " watchOrdersForSymbols() is not supported yet"
    );
  }

  async watchOHLCVForSymbols(
    symbolsAndTimeframes: string[][],
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
  ): Promise<Dictionary<Dictionary<OHLCV[]>>> {
    throw new NotSupported(
      this.id + " watchOHLCVForSymbols() is not supported yet"
    );
  }

  async unWatchOHLCVForSymbols(
    symbolsAndTimeframes: string[][],
    params = {}
  ): Promise<any> {
    throw new NotSupported(
      this.id + " unWatchOHLCVForSymbols() is not supported yet"
    );
  }
  ...
  async watchOrderBookForSymbols(
    ...
    throw new NotSupported(
      this.id + " watchOrderBookForSymbols() is not supported yet"
    );
  }
  ...
  async unWatchOrderBookForSymbols(
    ...
    throw new NotSupported(
      this.id + " unWatchOrderBookForSymbols() is not supported yet"
    );
  }
  ...
  async fetchDepositAddresses(
    ...
    throw new NotSupported(
      this.id + " fetchDepositAddresses() is not supported yet"
    );
  }
  ...
  async fetchOrderBookWs(
    symbol: string,
    limit: Int = undefined,
    params = {}
  ): Promise<OrderBook> {
    throw new NotSupported(
      this.id + " fetchOrderBookWs() is not supported yet"
    );
  }
  ...
  async fetchMarginMode(symbol: string, params = {}): Promise<MarginMode> {
    if (this.has["fetchMarginModes"]) {
      ...
      throw new NotSupported(
        this.id + " fetchMarginMode() is not supported yet"
      );
    }
  }

  async fetchMarginModes(
    symbols: Strings = undefined,
    params = {}
  ): Promise<MarginModes> {
    throw new NotSupported(
      this.id + " fetchMarginModes () is not supported yet"
    );
  }

  async fetchRestOrderBookSafe(symbol, limit = undefined, params = {}) {
    const fetchSnapshotMaxRetries = this.handleOption(
      "watchOrderBook",
      "maxRetries",
      3
    );
  ...
  async unWatchOrderBook(symbol: string, params = {}): Promise<any> {
    throw new NotSupported(
      this.id + " unWatchOrderBook() is not supported yet"
    );
  }
  ...
  async fetchTradingLimits(
    ...
    throw new NotSupported(
      this.id + " fetchTradingLimits() is not supported yet"
    );
  }
  ...
  parseDepositAddress(
    ...
    throw new NotSupported(
      this.id + " parseDepositAddress() is not supported yet"
    );
  }
  ...
  parseTransaction(
    ...
    throw new NotSupported(
      this.id + " parseTransaction() is not supported yet"
    );
  }
  ...
  parseLedgerEntry(item: Dict, currency: Currency = undefined): LedgerEntry {
    throw new NotSupported(
      this.id + " parseLedgerEntry() is not supported yet"
    );
  }
  ...
  async fetchCrossBorrowRates(params = {}): Promise<CrossBorrowRates> {
    throw new NotSupported(
      this.id + " fetchCrossBorrowRates() is not supported yet"
    );
  }

  async fetchIsolatedBorrowRates(params = {}): Promise<IsolatedBorrowRates> {
    throw new NotSupported(
      this.id + " fetchIsolatedBorrowRates() is not supported yet"
    );
  }

  parseMarketLeverageTiers(info, market: Market = undefined): LeverageTier[] {
    throw new NotSupported(
      this.id + " parseMarketLeverageTiers() is not supported yet"
    );
  }

  async fetchLeverageTiers(
    symbols: Strings = undefined,
    params = {}
  ): Promise<LeverageTiers> {
    throw new NotSupported(
      this.id + " fetchLeverageTiers() is not supported yet"
    );
  }
  ...
  parseFundingRateHistory(
    ...
    throw new NotSupported(
      this.id + " parseFundingRateHistory() is not supported yet"
    );
  }

  parseBorrowInterest(info: Dict, market: Market = undefined): BorrowInterest {
    throw new NotSupported(
      this.id + " parseBorrowInterest() is not supported yet"
    );
  }
  ...
  parseIsolatedBorrowRate(
    ...
    throw new NotSupported(
      this.id + " parseIsolatedBorrowRate() is not supported yet"
    );
  }
  ...
  parseWsOrderTrade(trade: Dict, market: Market = undefined): Trade {
    throw new NotSupported(
      this.id + " parseWsOrderTrade() is not supported yet"
    );
  }
  ...
  async fetchFundingRates(
    ...
    throw new NotSupported(
      this.id + " fetchFundingRates() is not supported yet"
    );
  }
  ...
  async fetchFundingIntervals(
    ...
    throw new NotSupported(
      this.id + " fetchFundingIntervals() is not supported yet"
    );
  }

  async watchFundingRate(symbol: string, params = {}): Promise<FundingRate> {
    throw new NotSupported(
      this.id + " watchFundingRate() is not supported yet"
    );
  }

  async watchFundingRates(
    symbols: string[],
    params = {}
  ): Promise<FundingRates> {
    throw new NotSupported(
      this.id + " watchFundingRates() is not supported yet"
    );
  }

  async watchFundingRatesForSymbols(
    symbols: string[],
    params = {}
  ): Promise<{}> {
  ...
  async createDepositAddress(
    ...
    throw new NotSupported(
      this.id + " createDepositAddress() is not supported yet"
    );
  }
  ...
  async fetchLongShortRatio(
    symbol: string,
    timeframe: Str = undefined,
    params = {}
  ): Promise<LongShortRatio> {
    throw new NotSupported(
      this.id + " fetchLongShortRatio() is not supported yet"
    );
  }
  ...
  async fetchLongShortRatioHistory(
    ...
    throw new NotSupported(
      this.id + " fetchLongShortRatioHistory() is not supported yet"
    );
  }

  async fetchMarginAdjustmentHistory(
    symbol: Str = undefined,
    type: Str = undefined,
    since: Num = undefined,
    limit: Num = undefined,
     ...
     */
    throw new NotSupported(
      this.id + " fetchMarginAdjustmentHistory() is not supported yet"
    );
  }
  ...
  async fetchDepositAddressesByNetwork(
    code: string,
    params = {}
  ): Promise<DepositAddress[]> {
    throw new NotSupported(
      this.id + " fetchDepositAddressesByNetwork() is not supported yet"
    );
  }
  ...
  async fetchOpenInterestHistory(
    ...
    throw new NotSupported(
      this.id + " fetchOpenInterestHistory() is not supported yet"
    );
  }

  async fetchOpenInterest(symbol: string, params = {}): Promise<OpenInterest> {
    throw new NotSupported(
      this.id + " fetchOpenInterest() is not supported yet"
    );
  }
  ...
  async fetchPaymentMethods(params = {}): Promise<{}> {
    throw new NotSupported(
      this.id + " fetchPaymentMethods() is not supported yet"
    );
  }
  ...
  featuresGenerator() {
    ...
    for (let i = 0; i < unifiedMarketTypes.length; i++) {
      ...
      if (!(marketType in initialFeatures)) {
        ...
        if (marketType === "spot") {
          this.features[marketType] = this.featuresMapper(
            initialFeatures,
            marketType,
            undefined
          );
          ...
          for (let j = 0; j < subTypes.length; j++) {
            ...
            this.features[marketType][subType] = this.featuresMapper(
              initialFeatures,
              marketType,
              subType
            );
  ...
  featuresMapper(
    ...
    if ("createOrder" in featuresObj) {
      const value = this.safeDict(
        featuresObj["createOrder"],
  ...
  orderbookChecksumMessage(symbol: Str) {
    return (
      symbol +
      " : " +
      'orderbook data checksum validation failed. You can reconnect by calling watchOrderBook again or you can mute the error by setting exchange.options["watchOrderBook"]["checksum"] = false'
    );
  }

  createNetworksByIdObject() {
    // automatically generate network-id-to-code mappings
    const networkIdsToCodesGenerated = this.invertFlatStringDictionary(
      this.safeValue(this.options, "networks", {})
    ); // invert defined networks dictionary
    this.options["networksById"] = this.extend(
      networkIdsToCodesGenerated,
      this.safeValue(this.options, "networksById", {})
    ...
    if (currencies !== undefined) {
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        const defaultCurrencyPrecision =
          this.precisionMode === DECIMAL_PLACES ? 8 : this.parseNumber("1e-8");
      ...
      for (let i = 0; i < codes.length; i++) {
        const code = codes[i];
        const groupedCurrenciesCode = this.safeList(
          groupedCurrencies,
          code,
          []
        );
  ...
  getDescribeForExtendedWsExchange(
    currentRestInstance: any,
    parentRestInstance: any,
    wsBaseDescribe: Dictionary<any>
  ) {
    const extendedRestDescribe = this.deepExtend(
      parentRestInstance.describe(),
      currentRestInstance.describe()
    );
    const superWithRestDescribe = this.deepExtend(
      extendedRestDescribe,
      wsBaseDescribe
    );
  ...
  safeOrder(order: Dict, market: Market = undefined): Order {
    ...
    let trades = [];
    const isTriggerOrSLTpOrder =
      this.safeString(order, "triggerPrice") !== undefined ||
      this.safeString(order, "stopLossPrice") !== undefined ||
      this.safeString(order, "takeProfitPrice") !== undefined;
    if (parseFilled || parseCost || shouldParseFees) {
      ...
      const tradesAreParsed =
        firstTrade !== undefined && "info" in firstTrade && "id" in firstTrade;
      if (!tradesAreParsed) {
    ...
    if (timeInForce === undefined) {
      if (
        ...
        this.safeString(order, "type") === "market"
      ) {
        timeInForce = "IOC";
      }
    ...
    const triggerPrice = this.parseNumber(
      this.safeString2(order, "triggerPrice", "stopPrice")
    );
    const takeProfitPrice = this.parseNumber(
      this.safeString(order, "takeProfitPrice")
    );
    const stopLossPrice = this.parseNumber(
      this.safeString(order, "stopLossPrice")
    );
  ...
  async borrowIsolatedMargin(
    ...
    throw new NotSupported(
      this.id + " borrowIsolatedMargin is not support yet"
    );
  }
  ...
  async fetchOHLCV(
    ...
    if (this.has["fetchTrades"]) {
      message =
        '. If you want to build OHLCV candles from trade executions data, visit https://github.com/ccxt/ccxt/tree/master/examples/ and see "build-ohlcv-bars" file';
    }
    throw new NotSupported(
      this.id + " fetchOHLCV() is not supported yet" + message
    );
  }
  ...
  async fetchWebEndpoint(
    method,
    endpointMethod,
    returnAsJson,
    startRegex = undefined,
    endRegex = undefined
  ) {
    let errorMessage = "";
  ...
  marketSymbols(
    ...
    if (symbols === undefined) {
      if (!allowEmpty) {
        throw new ArgumentsRequired(
          this.id + " empty list of symbols is not supported"
        );
      }
      return symbols;
    }
  ...
  parseBidsAsks(
    ...
    for (let i = 0; i < bidasks.length; i++) {
      result.push(
        this.parseBidAsk(bidasks[i], priceKey, amountKey, countOrIdKey)
      );
    }
    return result;
  }
  ...
  networkCodeToId(networkCode: string, currencyCode: Str = undefined): string {
    ...
    if (networkId === undefined) {
      if (currencyCode === undefined) {
        ...
        const defaultNetworkCodeReplacements = this.safeValue(
          this.options,
          "defaultNetworkCodeReplacements",
          {}
        );
        if (currencyCode in defaultNetworkCodeReplacements) {
          ...
          const replacementObject =
            defaultNetworkCodeReplacements[currencyCode]; // i.e. { 'ERC20': 'ETH' }
  ...
  networkIdToCode(
    networkId: Str = undefined,
    currencyCode: Str = undefined
  ): string {
    /**
     * @ignore
     * @method
    ...
    if (currencyCode !== undefined) {
      const defaultNetworkCodeReplacements = this.safeDict(
        this.options,
        "defaultNetworkCodeReplacements",
        {}
      );
      if (currencyCode in defaultNetworkCodeReplacements) {
        const replacementObject = this.safeDict(
          defaultNetworkCodeReplacements,
          currencyCode,
          {}
        );
    ...
    if (networkCode !== undefined) {
      if (responseNetworksLength === 0) {
        ...
        const networkId = isIndexedByUnifiedNetworkCode
          ? networkCode
          : this.networkCodeToId(networkCode, currencyCode);
      ...
      if (responseNetworksLength === 0) {
        throw new NotSupported(
          this.id + " - no networks were returned for " + currencyCode
        );
      } else {
        // if networkCode was not provided by user, then we try to use the default network (if it was defined in "defaultNetworks"), otherwise, we just return the first network entry
        const defaultNetworkCode = this.defaultNetworkCode(currencyCode);
        const defaultNetworkId = isIndexedByUnifiedNetworkCode
          ? defaultNetworkCode
          : this.networkCodeToId(defaultNetworkCode, currencyCode);
        chosenNetworkId =
  ...
  safePosition(position: Dict): Position {
    ...
    if (
      percentage === undefined &&
      unrealizedPnlString !== undefined &&
      initialMarginString !== undefined
    ) {
      // as it was done in all implementations ( aax, btcex, bybit, deribit, ftx, gate, kucoinfutures, phemex )
      const percentageString = Precise.stringMul(
        Precise.stringDiv(unrealizedPnlString, initialMarginString, 4),
        "100"
      );
  ...
  getSymbolsForMarketType(
    marketType: Str = undefined,
    subType: Str = undefined,
    symbolWithActiveStatus: boolean = true,
    symbolWithUnknownStatus: boolean = true
  ) {
    ...
    filteredMarkets = this.filterByArray(
      filteredMarkets,
      "active",
      activeStatuses,
      false
  ...
  async fetch2(
    ...
    [retries, params] = this.handleOptionAndParams(
      params,
      path,
      "maxRetriesOnFailure",
      0
    ...
    [retryDelay, params] = this.handleOptionAndParams(
      params,
      path,
      "maxRetriesOnFailureDelay",
      0
    );
  ...
  buildOHLCVC(
    ...
    for (let i = 0; i < oldest; i++) {
      ...
      if (
        candle === -1 ||
        openingTime >= this.sum(ohlcvs[candle][i_timestamp], ms)
      ) {
        ...
        ohlcvs[candle][i_high] = Math.max(
          ohlcvs[candle][i_high],
          trade["price"]
        );
        ohlcvs[candle][i_low] = Math.min(ohlcvs[candle][i_low], trade["price"]);
        ohlcvs[candle][i_close] = trade["price"];
        ohlcvs[candle][i_volume] = this.sum(
          ohlcvs[candle][i_volume],
          trade["amount"]
        );
  ...
  async fetchPositionsForSymbol(
     ...
     */
    throw new NotSupported(
      this.id + " fetchPositionsForSymbol() is not supported yet"
    );
  }

  async fetchPositionsForSymbolWs(
    symbol: string,
    params = {}
  ): Promise<Position[]> {
    /**
  ...
  async fetchPositionsRisk(
    symbols: Strings = undefined,
    params = {}
  ): Promise<Position[]> {
    throw new NotSupported(
      this.id + " fetchPositionsRisk() is not supported yet"
    );
  }
  ...
  async fetchBorrowInterest(
    ...
    limit: Int = undefined,
    params = {}
  ): Promise<BorrowInterest[]> {
    throw new NotSupported(
      this.id + " fetchBorrowInterest() is not supported yet"
    );
  }
  ...
  async fetchLedgerEntry(
    id: string,
    code: Str = undefined,
    params = {}
  ): Promise<LedgerEntry> {
    throw new NotSupported(
      this.id + " fetchLedgerEntry() is not supported yet"
    );
  }
  ...
  async fetchDepositWithdrawFees(
    codes: Strings = undefined,
    params = {}
  ): Promise<Dictionary<DepositWithdrawFeeNetwork>> {
    throw new NotSupported(
      this.id + " fetchDepositWithdrawFees() is not supported yet"
    );
  }

  async fetchDepositWithdrawFee(
    code: string,
    params = {}
  ): Promise<DepositWithdrawFeeNetwork> {
    if (!this.has["fetchDepositWithdrawFees"]) {
      throw new NotSupported(
        this.id + " fetchDepositWithdrawFee() is not supported yet"
      );
    }
  ...
  async fetchCrossBorrowRate(
    code: string,
    params = {}
  ): Promise<CrossBorrowRate> {
    await this.loadMarkets();
    if (!this.has["fetchBorrowRates"]) {
      throw new NotSupported(
        this.id + " fetchCrossBorrowRate() is not supported yet"
      );
    }
  ...
  async fetchIsolatedBorrowRate(
    symbol: string,
    params = {}
  ): Promise<IsolatedBorrowRate> {
    await this.loadMarkets();
    if (!this.has["fetchBorrowRates"]) {
      throw new NotSupported(
        this.id + " fetchIsolatedBorrowRate() is not supported yet"
      );
    }
  ...
  handleOptionAndParams(
    ...
    if (value !== undefined) {
      ...
      [methodName, params] = this.handleParamString(
        params,
        "callerMethodName",
        methodName
      );
  ...
  handleOptionAndParams2(
    ...
    [value2, params] = this.handleOptionAndParams(
      params,
      methodName1,
      optionName2,
      value
  ...
  handleMarketTypeAndParams(
    ...
    if (methodOptions !== undefined) {
      // user defined methodType takes precedence over defaultValue
  ...
  async fetchMarkPrice(symbol: string, params = {}): Promise<Ticker> {
    if (this.has["fetchMarkPrices"]) {
      ...
      if (ticker === undefined) {
        throw new NullResponse(
          this.id + " fetchMarkPrices() could not find a ticker for " + symbol
        );
      } else {
        return ticker as Ticker;
      }
      ...
      throw new NotSupported(
        this.id + " fetchMarkPrices() is not supported yet"
      );
    }
  }
  ...
  async fetchTickerWs(symbol: string, params = {}): Promise<Ticker> {
    if (this.has["fetchTickersWs"]) {
      ...
      if (ticker === undefined) {
        throw new NullResponse(
          this.id + " fetchTickerWs() could not find a ticker for " + symbol
        );
      } else {
        return ticker as Ticker;
      }
  ...
  async fetchUnifiedOrder(order, params = {}): Promise<Order> {
    return await this.fetchOrder(
      this.safeString(order, "id"),
      this.safeString(order, "symbol"),
      params
    );
  }
  ...
  async createTrailingAmountOrder(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    throw new NotSupported(
      this.id + " createTrailingAmountOrder() is not supported yet"
    );
  }

  async createTrailingAmountOrderWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    if (this.has["createTrailingAmountOrderWs"]) {
      return await this.createOrderWs(
        symbol,
        type,
        side,
        amount,
    ...
    throw new NotSupported(
      this.id + " createTrailingAmountOrderWs() is not supported yet"
    );
  }

  async createTrailingPercentOrder(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    throw new NotSupported(
      this.id + " createTrailingPercentOrder() is not supported yet"
    );
  }

  async createTrailingPercentOrderWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    throw new NotSupported(
      this.id + " createTrailingPercentOrderWs() is not supported yet"
    );
  }
  ...
  async createMarketOrderWithCost(
     ...
     */
    if (
      this.has["createMarketOrderWithCost"] ||
      (this.has["createMarketBuyOrderWithCost"] &&
        this.has["createMarketSellOrderWithCost"])
    ) {
      return await this.createOrder(symbol, "market", side, cost, 1, params);
    }
    throw new NotSupported(
      this.id + " createMarketOrderWithCost() is not supported yet"
    );
  }
  ...
  async createMarketBuyOrderWithCost(
     ...
     */
    if (
      this.options["createMarketBuyOrderRequiresPrice"] ||
      this.has["createMarketBuyOrderWithCost"]
    ) {
      return await this.createOrder(symbol, "market", "buy", cost, 1, params);
    }
    throw new NotSupported(
      this.id + " createMarketBuyOrderWithCost() is not supported yet"
    );
  }

  async createMarketSellOrderWithCost(
    symbol: string,
    cost: number,
    params = {}
  ): Promise<Order> {
     ...
     */
    if (
      this.options["createMarketSellOrderRequiresPrice"] ||
      this.has["createMarketSellOrderWithCost"]
    ) {
      return await this.createOrder(symbol, "market", "sell", cost, 1, params);
    }
    throw new NotSupported(
      this.id + " createMarketSellOrderWithCost() is not supported yet"
    );
  }

  async createMarketOrderWithCostWs(
    symbol: string,
    side: OrderSide,
    cost: number,
    params = {}
     ...
     */
    if (
      this.has["createMarketOrderWithCostWs"] ||
      (this.has["createMarketBuyOrderWithCostWs"] &&
        this.has["createMarketSellOrderWithCostWs"])
    ) {
      return await this.createOrderWs(symbol, "market", side, cost, 1, params);
    }
    throw new NotSupported(
      this.id + " createMarketOrderWithCostWs() is not supported yet"
    );
  }
  ...
  async createTriggerOrder(
    ...
    if (triggerPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createTriggerOrder() requires a triggerPrice argument"
      );
    }
    ...
    throw new NotSupported(
      this.id + " createTriggerOrder() is not supported yet"
    );
  }

  async createTriggerOrderWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    if (triggerPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createTriggerOrderWs() requires a triggerPrice argument"
      );
    }
    params["triggerPrice"] = triggerPrice;
    ...
    throw new NotSupported(
      this.id + " createTriggerOrderWs() is not supported yet"
    );
  }
  ...
  async createStopLossOrder(
    ...
    if (stopLossPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createStopLossOrder() requires a stopLossPrice argument"
      );
    }
    ...
    throw new NotSupported(
      this.id + " createStopLossOrder() is not supported yet"
    );
  }

  async createStopLossOrderWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    if (stopLossPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createStopLossOrderWs() requires a stopLossPrice argument"
      );
    }
    params["stopLossPrice"] = stopLossPrice;
    ...
    throw new NotSupported(
      this.id + " createStopLossOrderWs() is not supported yet"
    );
  }
  ...
  async createTakeProfitOrder(
    ...
    if (takeProfitPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createTakeProfitOrder() requires a takeProfitPrice argument"
      );
    }
    ...
    throw new NotSupported(
      this.id + " createTakeProfitOrder() is not supported yet"
    );
  }

  async createTakeProfitOrderWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    throw new NotSupported(
      this.id + " createTakeProfitOrderWs() is not supported yet"
    );
  }

  async createOrderWithTakeProfitAndStopLoss(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    price: Num = undefined,
    takeProfit: Num = undefined,
    stopLoss: Num = undefined,
    params = {}
  ): Promise<Order> {
    /**
     * @method
     ...
     * @returns {object} an [order structure]{@link https://docs.ccxt.com/#/?id=order-structure}
     */
    params = this.setTakeProfitAndStopLossParams(
      symbol,
      type,
      side,
      amount,
    ...
    throw new NotSupported(
      this.id + " createOrderWithTakeProfitAndStopLoss() is not supported yet"
    );
  }
  ...
  setTakeProfitAndStopLossParams(
    ...
    const takeProfitLimitPrice = this.safeString(
      params,
  ...
  async createOrderWithTakeProfitAndStopLossWs(
    symbol: string,
    type: OrderType,
    side: OrderSide,
    amount: number,
    ...
    throw new NotSupported(
      this.id + " createOrderWithTakeProfitAndStopLossWs() is not supported yet"
    );
  }
  ...
  async cancelAllOrdersAfter(timeout: Int, params = {}): Promise<{}> {
    throw new NotSupported(
      this.id + " cancelAllOrdersAfter() is not supported yet"
    );
  }

  async cancelOrdersForSymbols(
    orders: CancellationRequest[],
    params = {}
  ): Promise<{}> {
    throw new NotSupported(
      this.id + " cancelOrdersForSymbols() is not supported yet"
    );
  }

  async cancelAllOrdersWs(symbol: Str = undefined, params = {}): Promise<{}> {
    throw new NotSupported(
      this.id + " cancelAllOrdersWs() is not supported yet"
    );
  }
  ...
  async fetchOrderTrades(
    ...
    throw new NotSupported(
      this.id + " fetchOrderTrades() is not supported yet"
    );
  }
  ...
  async fetchOpenOrdersWs(
    ...
    throw new NotSupported(
      this.id + " fetchOpenOrdersWs() is not supported yet"
    );
  }
  ...
  async fetchCanceledAndClosedOrders(
    symbol: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " fetchCanceledAndClosedOrders() is not supported yet"
    );
  }

  async fetchClosedOrdersWs(
    symbol: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
    ...
    throw new NotSupported(
      this.id + " fetchClosedOrdersWs() is not supported yet"
    );
  }
  ...
  async fetchMyLiquidations(
    ...
    throw new NotSupported(
      this.id + " fetchMyLiquidations() is not supported yet"
    );
  }
  ...
  async fetchLiquidations(
    ...
    throw new NotSupported(
      this.id + " fetchLiquidations() is not supported yet"
    );
  }

  async fetchMyTradesWs(
    symbol: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
  ...
  async fetchOptionChain(code: string, params = {}): Promise<OptionChain> {
    throw new NotSupported(
      this.id + " fetchOptionChain() is not supported yet"
    );
  }
  ...
  async fetchConvertQuote(
    ...
    throw new NotSupported(
      this.id + " fetchConvertQuote() is not supported yet"
    );
  }
  ...
  async fetchDepositsWithdrawals(
     ...
     */
    throw new NotSupported(
      this.id + " fetchDepositsWithdrawals() is not supported yet"
    );
  }
  ...
  async fetchWithdrawalsWs(
    ...
    throw new NotSupported(
      this.id + " fetchWithdrawalsWs() is not supported yet"
    );
  }
  ...
  async fetchFundingRateHistory(
    ...
    throw new NotSupported(
      this.id + " fetchFundingRateHistory() is not supported yet"
    );
  }
  ...
  async fetchFundingHistory(
    ...
    throw new NotSupported(
      this.id + " fetchFundingHistory() is not supported yet"
    );
  }
  ...
  async closeAllPositions(params = {}): Promise<Position[]> {
    throw new NotSupported(
      this.id + " closeAllPositions() is not supported yet"
    );
  }
  ...
  async fetchDepositAddress(
    ...
    if (this.has["fetchDepositAddresses"]) {
      ...
      const addressStructures = await this.fetchDepositAddressesByNetwork(
        code,
        params
      );
  ...
  createExpiredOptionMarket(symbol: string): MarketInterface {
    throw new NotSupported(
      this.id + " createExpiredOptionMarket () is not supported yet"
    );
  }
    ...
    if (result === "0") {
      throw new InvalidOrder(
        this.id +
          ...
          this.numberToString(market["precision"]["price"])
      );
    }
    return result;
  }
    ...
    if (result === "0") {
      throw new InvalidOrder(
        this.id +
          ...
          this.numberToString(market["precision"]["amount"])
      );
    }
    return result;
  }
  ...
  currencyToPrecision(code: string, fee, networkCode = undefined) {
    ...
    if (precision === undefined) {
      ...
      const roundingMode = this.safeInteger(
        this.options,
        "currencyToPrecisionRoundingMode",
        ROUND
      );
      return this.decimalToPrecision(
        fee,
  ...
  async createPostOnlyOrderWs(
    ...
    if (!this.has["createPostOnlyOrderWs"]) {
      throw new NotSupported(
        this.id + "createPostOnlyOrderWs() is not supported yet"
      );
    }
    const query = this.extend(params, { postOnly: true });
  ...
  async createReduceOnlyOrderWs(
    ...
    if (!this.has["createReduceOnlyOrderWs"]) {
      throw new NotSupported(
        this.id + "createReduceOnlyOrderWs() is not supported yet"
      );
    }
    const query = this.extend(params, { reduceOnly: true });
    return await this.createOrderWs(symbol, type, side, amount, price, query);
  ...
  async createStopOrderWs(
    ...
    if (!this.has["createStopOrderWs"]) {
      throw new NotSupported(
        this.id + " createStopOrderWs() is not supported yet"
      );
    }
    if (stopPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " createStopOrderWs() requires a stopPrice argument"
      );
    }
    const query = this.extend(params, { stopPrice: stopPrice });
    return await this.createOrderWs(symbol, type, side, amount, price, query);
  ...
  async createStopLimitOrderWs(
    ...
    if (!this.has["createStopLimitOrderWs"]) {
      throw new NotSupported(
        this.id + " createStopLimitOrderWs() is not supported yet"
      );
    }
    const query = this.extend(params, { stopPrice: stopPrice });
    return await this.createOrderWs(
  ...
  async createStopMarketOrderWs(
    ...
    if (!this.has["createStopMarketOrderWs"]) {
      throw new NotSupported(
        this.id + " createStopMarketOrderWs() is not supported yet"
      );
    }
    const query = this.extend(params, { stopPrice: stopPrice });
    return await this.createOrderWs(
  ...
  parseLastPrices(
    ...
    if (Array.isArray(pricesData)) {
      for (let i = 0; i < pricesData.length; i++) {
        const priceData = this.extend(
          this.parseLastPrice(pricesData[i]),
          params
        );
      ...
      for (let i = 0; i < marketIds.length; i++) {
        ...
        const priceData = this.extend(
          this.parseLastPrice(pricesData[marketId], market),
          params
        );
        results.push(priceData);
      }
  ...
  parseLongShortRatio(info: Dict, market: Market = undefined): LongShortRatio {
    throw new NotSupported(
      this.id + " parseLongShortRatio() is not supported yet"
    );
  }

  parseLongShortRatioHistory(
    response,
    market = undefined,
    since: Int = undefined,
    limit: Int = undefined
  ...
  handlePostOnly(
    ...
    exchangeSpecificPostOnlyOption: boolean,
    params: any = {}
  ) {
    /**
     * @ignore
  ...
  async fetchTradingFeesWs(params = {}): Promise<TradingFees> {
    throw new NotSupported(
      this.id + " fetchTradingFeesWs() is not supported yet"
    );
  }
  ...
  async fetchConvertCurrencies(params = {}): Promise<Currencies> {
    throw new NotSupported(
      this.id + " fetchConvertCurrencies() is not supported yet"
    );
  }
  ...
  async fetchFundingInterval(
    ...
    if (this.has["fetchFundingIntervals"]) {
      ...
      if (!market["contract"]) {
        throw new BadSymbol(
          this.id + " fetchFundingInterval() supports contract markets only"
        );
      }
      ...
      if (rate === undefined) {
        throw new NullResponse(
          this.id + " fetchFundingInterval() returned no data for " + symbol
        );
      } else {
        return rate;
      }
      ...
      throw new NotSupported(
        this.id + " fetchFundingInterval() is not supported yet"
      );
    }
  }
  ...
  checkRequiredArgument(
    ...
    if (
      argument === undefined ||
      (optionsLength > 0 && !this.inArray(argument, options))
    ) {
  ...
  parseDepositWithdrawFees(
    ...
    for (let i = 0; i < responseKeys.length; i++) {
      ...
      if (codes === undefined || this.inArray(code, codes)) {
        depositWithdrawFees[code] = this.parseDepositWithdrawFee(
          dictionary,
          currency
        );
      }
  ...
  parseDepositWithdrawFee(fee, currency: Currency = undefined): any {
    throw new NotSupported(
      this.id + " parseDepositWithdrawFee() is not supported yet"
    );
  }
  ...
  handleMaxEntriesPerRequestAndParams(
    method: string,
    maxEntriesPerRequest: Int = undefined,
    params = {}
  ): [Int, any] {
    let newMaxEntriesPerRequest = undefined;
    [newMaxEntriesPerRequest, params] = this.handleOptionAndParams(
      params,
      method,
      "maxEntriesPerRequest"
    );
    if (
      newMaxEntriesPerRequest !== undefined &&
  ...
  async fetchPaginatedCallDynamic(
    method: string,
    symbol: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    ...
    [maxCalls, params] = this.handleOptionAndParams(
      params,
      method,
      "paginationCalls",
      10
    ...
    [maxRetries, params] = this.handleOptionAndParams(
      params,
      method,
      "maxRetries",
      3
    ...
    [paginationDirection, params] = this.handleOptionAndParams(
      params,
      method,
      "paginationDirection",
      "backward"
    ...
    [maxEntriesPerRequest, params] = this.handleMaxEntriesPerRequestAndParams(
      method,
      maxEntriesPerRequest,
      params
    );
        ...
        if (paginationDirection === "backward") {
          ...
          if (this.verbose) {
            let backwardMessage =
              ...
              this.numberToString(calls) +
              " method " +
              method +
              " response length " +
              this.numberToString(responseLength);
            if (paginationTimestamp !== undefined) {
              backwardMessage +=
                " timestamp " + this.numberToString(paginationTimestamp);
            }
  ...
  async fetchPaginatedCallDeterministic(
    method: string,
    symbol: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    timeframe: Str = undefined,
    params = {},
    maxEntriesPerRequest = undefined
  ): Promise<any> {
    let maxCalls = undefined;
    [maxCalls, params] = this.handleOptionAndParams(
      params,
  ...
  async fetchPaginatedCallCursor(
    method: string,
    symbol: Str = undefined,
    since = undefined,
    limit = undefined,
    params = {},
    cursorReceived = undefined,
    cursorSent = undefined,
    cursorIncrement = undefined,
    maxEntriesPerRequest = undefined
  ): Promise<any> {
    let maxCalls = undefined;
    [maxCalls, params] = this.handleOptionAndParams(
    ...
    while (i < maxCalls) {
      try {
        ...
        if (method === "fetchAccounts") {
          ...
          method === "getLeverageTiersPaginated" ||
  ...
  async fetchPaginatedCallIncremental(
    method: string,
    symbol: Str = undefined,
    since = undefined,
    limit = undefined,
  ...
  parseLiquidation(liquidation, market: Market = undefined): Liquidation {
    throw new NotSupported(
      this.id + " parseLiquidation () is not supported yet"
    );
  }
  ...
  parseOptionChain(
    response: object[],
    currencyKey: Str = undefined,
    symbolKey: Str = undefined
  ): OptionChain {
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      optionStructures[market["symbol"]] = this.parseOption(
        info,
        currency,
        market
      );
  ...
  parseMarginModes(
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      if (symbols === undefined || this.inArray(market["symbol"], symbols)) {
        marginModeStructures[market["symbol"]] = this.parseMarginMode(
          info,
          market
        );
      }
  ...
  parseMarginMode(marginMode: Dict, market: Market = undefined): MarginMode {
    throw new NotSupported(
      this.id + " parseMarginMode () is not supported yet"
    );
  }
  ...
  parseConversions(
    conversions: any[],
    code: Str = undefined,
    fromCurrencyKey: Str = undefined,
    toCurrencyKey: Str = undefined,
    since: Int = undefined,
    limit: Int = undefined,
    params = {}
  ): Conversion[] {
    ...
    for (let i = 0; i < conversions.length; i++) {
      ...
      const conversion = this.extend(
        this.parseConversion(entry, fromCurrency, toCurrency),
        params
      );
  ...
  parseConversion(
    ...
    throw new NotSupported(
      this.id + " parseConversion () is not supported yet"
    );
  }
  ...
  convertExpireDate(date: string): string {
    ...
    const reconstructedDate =
      "20" + year + "-" + month + "-" + day + "T00:00:00Z";
  ...
  async fetchPositionHistory(
    ...
    if (this.has["fetchPositionsHistory"]) {
      const positions = await this.fetchPositionsHistory(
        [symbol],
        since,
        limit,
        params
      ...
      throw new NotSupported(
        this.id + " fetchPositionHistory () is not supported yet"
      );
    }
  }
  ...
  async fetchPositionsHistory(
     ...
     */
    throw new NotSupported(
      this.id + " fetchPositionsHistory () is not supported yet"
    );
  }
  ...
  parseMarginModification(
    ...
    throw new NotSupported(
      this.id + " parseMarginModification() is not supported yet"
    );
  }

  parseMarginModifications(
    response: object[],
    symbols: Strings = undefined,
    symbolKey: Str = undefined,
    marketType: MarketType = undefined
  ...
  cleanCache(subscription: Dict) {
    ...
    const symbolsLength = symbols.length;
    if (topic === "ohlcv") {
      const symbolsAndTimeFrames = this.safeList(
        subscription,
